...
class GainCompression(metaclass=Metaclass_GainCompression):
    ...
    def __init__(self, **kwargs):
        ...
        if 'compression_gains' not in kwargs:
            ...
            self.compression_gains = numpy.array(kwargs.get('compression_gains'), dtype=numpy.float32)
            assert self.compression_gains.shape == (3, )
        if 'spectral_damper_hpf' not in kwargs:
            self.spectral_damper_hpf = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.spectral_damper_hpf = numpy.array(kwargs.get('spectral_damper_hpf'), dtype=numpy.float32)
            assert self.spectral_damper_hpf.shape == (3, )
        if 'spectral_damper_out' not in kwargs:
            self.spectral_damper_out = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.spectral_damper_out = numpy.array(kwargs.get('spectral_damper_out'), dtype=numpy.float32)
            assert self.spectral_damper_out.shape == (3, )
