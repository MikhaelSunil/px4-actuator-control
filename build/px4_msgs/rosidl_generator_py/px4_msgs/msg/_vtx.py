...
class Vtx(metaclass=Metaclass_Vtx):
    ...
    def __init__(self, **kwargs):
        ...
        if 'band_name' not in kwargs:
            ...
            self.band_name = numpy.array(kwargs.get('band_name'), dtype=numpy.uint8)
            assert self.band_name.shape == (12, )
        self.power_level = kwargs.get('power_level', int())
        if 'power_label' not in kwargs:
            self.power_label = numpy.zeros(4, dtype=numpy.uint8)
        else:
            self.power_label = numpy.array(kwargs.get('power_label'), dtype=numpy.uint8)
            assert self.power_label.shape == (4, )
